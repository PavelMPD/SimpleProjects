...
function ActivateCsEditor(editor) {
    ...
    var config = {
        parserfile: [
        "<%=GetScriptName("parsexml.js")%>"
        ,"<%=GetScriptName("parsecss.js")%>"
        ,"<%=GetScriptName("tokenizejavascript.js")%>"
        ,"<%=GetScriptName("parsejavascript.js")%>"
        ,"<%=GetScriptName("parsehtmlmixed.js")%>"
        ],
			
        stylesheet: [
			"<%=GetScriptName("xmlcolors.css")%>"
			,"<%=GetScriptName("jscolors.css")%>"
			,"<%=GetScriptName("csscolors.css")%>"
			],
        //path: "cseditor/",
        height: "100%",
    ...
    proxy = new CodeMirror(replace, config);//CodeMirror.replace(editor.elemText)

    editor.onCallback = function() {
        var code = proxy.getCode();
    ...
    proxy.wrapping.id = "csproxywrapping";
}
...
function HideCompilerPanel()
{
	...
	<px:PXDataSource ID="ds" runat="server" Visible="true" TypeName="PX.SM.GraphAspxEdit"
		...
		<CallbackCommands>
			...
			<px:PXDSCallbackCommand Name="actionCheckout" BlockPage="True" CommitChanges="True"
			                        RepaintControls="None" RepaintControlsIDs="FormEditContent" CommitChangesIDs="FormEditContent" Visible="False"/>
			
			<px:PXDSCallbackCommand Name="actionRemoveFile" BlockPage="True" CommitChanges="True"
			                        RepaintControls="None" RepaintControlsIDs="FormEditContent" CommitChangesIDs="FormEditContent" Visible="False" />
...
<asp:Content ID="Content2" ContentPlaceHolderID="phF" runat="Server">
	<px:PXFormView ID="FormEditContent" runat="server" DataMember="Filter"
		DataSourceID="ds" Width="100%"  Caption="Project"
		>
		<Template>
			<px:PXLayoutRule runat="server" ControlSize="M" LabelsWidth="SM" 
			                 StartColumn="True"/>
			
			<px:PXSelector ID="PXSelector1" CommitChanges="True" runat="server" DataSourceID="ds"
			               DataField="SelectFile" AutoGenerateColumns="True" ValueField="VirtualPath"/>
			
<%--			<px:PXLayoutRule runat="server" ControlSize="SM" LabelsWidth="S" 
			                 StartColumn="True"/>
			
			<px:PXTextEdit runat="server" DataField="ExternalFile" ID="ExternalFileID" />--%>
			<px:PXLayoutRule runat="server" ControlSize="SM" LabelsWidth="S" 
			                 StartColumn="True"/>
			
			<px:PXTextEdit runat="server" DataField="WorkingProject" ID="WorkingProjectID" />
			<px:PXLayoutRule runat="server" ColumnSpan="3" StartRow="True"/>
			
			<px:PXTextEdit ID="EventEditBox" runat="server" DataField="Source" Style="position: absolute;
				left: 0px; top: 0px; width: 200px; height: 20px; margin: 0px; border: 0px; display: none;
				visibility: hidden" TextMode="MultiLine" Font-Names="Courier New" Font-Size="10pt"
