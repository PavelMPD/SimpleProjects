	...
	public class CountryMaint : PXGraph<CountryMaint, Country>
	{
		...
		protected virtual void Country_RowPersisting(PXCache cache, PXRowPersistingEventArgs e)
		{
			if ((e.Operation & PXDBOperation.Command) == PXDBOperation.Insert || (e.Operation & PXDBOperation.Command) == PXDBOperation.Update)
			{
				...
				{
					cache.RaiseExceptionHandling("Description", e.Row, country.Description, new PXSetPropertyException("Country Name can't be empty"));
				}
				if (string.IsNullOrEmpty(country.CountryID) || string.IsNullOrEmpty(country.CountryID.Trim()))
				{
					cache.RaiseExceptionHandling("CountryID", e.Row, country.CountryID, new PXSetPropertyException("CountryID can't be empty"));
				}
			}	
		}
