...
namespace PX.TM
{
	...
	public class EPCalendarEnq : PXGraph<EPCalendarEnq>
	{
		...
		public PXSelectJoin<EPActivity,
			LeftJoin<Attendee, On<Attendee.userID, Equal<Current<AccessInfo.userID>>,
					And<Attendee.eventID, Equal<EPActivity.taskID>>>>,
			Where2<Where<EPActivity.classID, Equal<CRActivityClass.events>>,
				And<Where<EPActivity.startDate, GreaterEqual<Current<EPCalendarFilter.startDate>>,
					And<EPActivity.startDate, LessEqual<Current<EPCalendarFilter.endDate>>,
						And<Where<EPActivity.createdByID, Equal<Current<AccessInfo.userID>>, Or<Attendee.userID, IsNotNull>>>>>>>,
		...
		public virtual IEnumerable CreateNew(PXAdapter adapter)
		{
			...
			var newevent = (EPActivity)eventMain.Events.Cache.Insert();
