...
namespace PX.Objects.EP
{
    ...
    public class EquipmentTimeCardMaint : PXGraph<EquipmentTimeCardMaint, EPEquipmentTimeCard>
    {
        ...
        [PXViewName(Messages.TimeCardDetails)]
        ...
        public PXAction<EPEquipmentTimeCard> assign;
        [PXUIField(DisplayName = Messages.Assign)]
        [PXButton]
        protected virtual IEnumerable Assign(PXAdapter adapter)
        {
            var mapId = EpSetup.Current.EquipmentTimeCardAssignmentMapID;
            if (mapId == null)
				throw new PXSetPropertyException(Messages.EmptyEquipmentTimecardAssignMap, "Time & Expenses Preferences");
        ...
        protected virtual IEnumerable Release(PXAdapter adapter)
        {
            ...
            foreach (EPEquipmentTimeCard item in adapter.Get<EPEquipmentTimeCard>())
            {
        ...
        protected virtual void ProcessRegularTimecard(PM.RegisterEntry releaseGraph, EPEquipmentTimeCard timecard)
        {
            ...
            foreach (EPEquipmentDetail detail in Details.Select())
            {
                ...
                InsertPMTran(releaseGraph, detail, equipment.SetupRateItemID, detail.SetupTime, GetSetupCost(equipment.EquipmentID, detail.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                InsertPMTran(releaseGraph, detail, equipment.RunRateItemID, detail.RunTime, GetRunCost(equipment.EquipmentID, detail.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                InsertPMTran(releaseGraph, detail, equipment.SuspendRateItemID, detail.SuspendTime, GetSuspendCost(equipment.EquipmentID, detail.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
            }
        }
        ...
        protected virtual void ProcessCorrectingTimecard(PM.RegisterEntry releaseGraph, EPEquipmentTimeCard timecard)
        {
            EPEquipment equipment = PXSelect<EPEquipment, Where<EPEquipment.equipmentID, Equal<Required<EPEquipment.equipmentID>>>>.Select(this, timecard.EquipmentID);
            if (equipment == null)
                throw new PXException("Failed to select Equipment.");
            ...
            foreach (PXResult<EPEquipmentDetailOrig, EPEquipmentDetailEx> res in selectDeletedItems.View.SelectMultiBound(new object[] { timecard }))
            {
                ...
                if (orig.SetupTime.GetValueOrDefault() > 0)
                {
                    ...
                    InsertPMTran(releaseGraph, orig, equipment.SetupRateItemID, -orig.SetupTime, GetSetupCost(equipment.EquipmentID, orig.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                }
                ...
                if (orig.RunTime.GetValueOrDefault() > 0)
                {
                    ...
                    InsertPMTran(releaseGraph, orig, equipment.RunRateItemID, -orig.RunTime, GetRunCost(equipment.EquipmentID, orig.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                }
                ...
                if (orig.SuspendTime.GetValueOrDefault() > 0)
                {
                    ...
                    InsertPMTran(releaseGraph, orig, equipment.SuspendRateItemID, -orig.SuspendTime, GetSuspendCost(equipment.EquipmentID, orig.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                }
            }
            ...
            foreach (EPEquipmentDetail item in selectAddedItems.View.SelectMultiBound(new object[] { timecard }))
            {
                if (item.SetupTime.GetValueOrDefault() > 0)
                {
                    ...
                    InsertPMTran(releaseGraph, item, equipment.SetupRateItemID, item.SetupTime, GetSetupCost(equipment.EquipmentID, item.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                }
                ...
                if (item.RunTime.GetValueOrDefault() > 0)
                {
                    ...
                    InsertPMTran(releaseGraph, item, equipment.RunRateItemID, item.RunTime, GetRunCost(equipment.EquipmentID, item.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                }
                ...
                if (item.SuspendTime.GetValueOrDefault() > 0)
                {
                    ...
                    InsertPMTran(releaseGraph, item, equipment.SuspendRateItemID, item.SuspendTime, GetSuspendCost(equipment.EquipmentID, item.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                }
            }
            ...
            foreach (PXResult<EPEquipmentDetailOrig, EPEquipmentDetailEx> res in selectModifiedItems.View.SelectMultiBound(new object[] { timecard }))
            {
                ...
                if (item.SetupTime.GetValueOrDefault() - orig.SetupTime.GetValueOrDefault() != 0)
                {
                    ...
                    InsertPMTran(releaseGraph, item, equipment.SetupRateItemID, item.SetupTime.GetValueOrDefault() - orig.SetupTime.GetValueOrDefault(), GetSetupCost(equipment.EquipmentID, item.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                }
                ...
                if (item.RunTime.GetValueOrDefault() - orig.RunTime.GetValueOrDefault() != 0)
                {
                    ...
                    InsertPMTran(releaseGraph, item, equipment.RunRateItemID, item.RunTime.GetValueOrDefault() - orig.RunTime.GetValueOrDefault(), GetRunCost(equipment.EquipmentID, item.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                }
                ...
                if (item.SuspendTime.GetValueOrDefault() - orig.SuspendTime.GetValueOrDefault() != 0)
                {
                    ...
                    InsertPMTran(releaseGraph, item, equipment.SuspendRateItemID, item.SuspendTime.GetValueOrDefault() - orig.SuspendTime.GetValueOrDefault(), GetSuspendCost(equipment.EquipmentID, item.ProjectID), equipment.EquipmentID, Setup.Current.CopyNotesPM == true, Setup.Current.CopyFilesPM == true);
                }
            }
        }
        ...
        protected virtual void InsertPMTran(RegisterEntry pmGraph, EPEquipmentDetail row, int? inventoryID, int? minutes, decimal? rate, int? equipmentID, bool copyNotes, bool copyFiles)
        {
            ...
            if (item != null && minutes != null)
            {
                ...
                if (copyNotes)
                {
                    string note = PXNoteAttribute.GetNote(Details.Cache, row);
                    if (note != null)
                        PXNoteAttribute.SetNote(pmGraph.Transactions.Cache, tran, note);
                }
                if (copyFiles)
                {
                    Guid[] files = PXNoteAttribute.GetFileNotes(Details.Cache, row);
