...
namespace PX.Objects.IN
{
	public class INAdjustmentEntry : PXGraph<INAdjustmentEntry, INRegister>
	{
		...
		{
			object current = null;
			if (viewName.ToLower() == "transactions")
			{
				current = transactions.Cache.Current;
			}
			...
			if (current != null)
			{
				transactions.Cache.Current = current;
			}
			return ret;
		}
		...
		protected virtual void DefaultUnitCost(PXCache sender, PXFieldUpdatedEventArgs e)
		{
			...
			if (item.ValMethod == INValMethod.Specific && string.IsNullOrEmpty(((INTran)e.Row).LotSerialNbr) == false)
			{
				...
				{
					UnitCost = PXDBPriceCostAttribute.Round(sender, (decimal)(status.TotalCost / status.QtyOnHand));
				}
			} 
			...
			if (UnitCost != null && (decimal)UnitCost != 0m)
			{
				...
				if ((decimal?)sender.GetValue<INTran.qty>(e.Row) == 0m)
				{
