...
namespace PX.Objects.DR
{
	...
	public class ScheduleMaint : PXGraph<ScheduleMaint>
	{
				...
				switch (sc.Module)
				{
					case BatchModule.AR:
						...
						if (invoice != null)
						{
							...
							throw new PXRedirectRequiredException(arTarget, "ViewDocument"){Mode = PXBaseRedirectException.WindowMode.NewWindow};
						}
						break;
					case BatchModule.AP:
						...
						APInvoice invoice2 = PXSelect<APInvoice, Where<APInvoice.docType, Equal<Current<DRScheduleDetail.docType>>, And<APInvoice.refNbr, Equal<Current<DRScheduleDetail.refNbr>>>>>.Select(this);
						if (invoice2 != null)
						{
							apTarget.Document.Current = invoice2;
							throw new PXRedirectRequiredException(apTarget, "ViewInvoice") { Mode = PXBaseRedirectException.WindowMode.NewWindow };
						}
						break;
				}
			}
		...
		{
			DRScheduleDetail row = e.Row as DRScheduleDetail;
			if (row != null && Schedule.Cache.AllowUpdate)
			{
				DRSchedule sc = Schedule.Select();
				if (sc != null)
				{
					sc.Module = row.Module;
					sc.DocType = row.DocType;
					sc.RefNbr = row.RefNbr;
					sc.LineNbr = row.LineNbr;
					sc.DocDate = row.DocDate;
					sc.FinPeriodID = row.FinPeriodID;
		...
		private void CreateTransactions(DRDeferredCode defCode, int? branchID)
		{
			if (Document.Current != null && Document.Current.Status == DRScheduleStatus.Draft)
			{
				...
				{
					IList<DRScheduleTran> tranList = DRProcess.GenerateTransactions(this, Schedule.Current, Document.Current, defCode, branchID);
		...
		internal void ReleaseCustomSchedule()
		{
			if (Document.Current != null && Document.Current.Status == DRScheduleStatus.Draft)
			{
		...
		private void CreateIncomingTransaction()
		{
			...
			tran.LineNbr = 0;
			tran.Module = Document.Current.Module;
		...
		public virtual void RebuildProjections(int? branchID)
		{
			...
			foreach (DRScheduleTran tran in existingTrans)
			{
				...
				balance -= tran.Amount.Value;
			}

			DRSchedule sc = PXSelect<DRSchedule, Where<DRSchedule.scheduleID, Equal<Current<DRScheduleDetail.scheduleID>>>>.Select(this);
			DRDeferredCode defCode = DeferredCode.Select();

			IList<DRScheduleTran> tranList = DRProcess.GenerateTransactions(this, sc, Document.Current, defCode, branchID);
