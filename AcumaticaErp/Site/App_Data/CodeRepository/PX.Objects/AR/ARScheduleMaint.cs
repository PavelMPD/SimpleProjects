...
{
	public class ARScheduleMaint : PXGraph<ARScheduleMaint, Schedule>
	{
        ...
        [PXSelector(typeof(Search2<Schedule.scheduleID,
            ...
            Where<Schedule.module, Equal<BatchModule.moduleAR>,
		...
		public PXSelectJoin<Schedule,
			...
			And<Not<Match<ARRegisterAccess, Current<AccessInfo.userName>>>>>>>,
			Where<Schedule.module, Equal<BatchModule.moduleAR>,
			And<ARRegisterAccess.docType, IsNull>>> Schedule_Header;
		public PXSelect<ARRegister, Where<ARRegister.scheduleID, Equal<Current<Schedule.scheduleID>>, And<ARRegister.scheduled, Equal<boolFalse>>>> Document_History;
				...
				else
				{
					...
					else
					{
						PXLongOperation.StartOperation(this,
							delegate()
							{
								ARScheduleProcess sp = PXGraph.CreateInstance<ARScheduleProcess>();
        ...
        protected virtual void DocumentSelection_RowInserted(PXCache cache, PXRowInsertedEventArgs e)
        {
            ...
            if (docSelection != null && !String.IsNullOrWhiteSpace(docSelection.DocType) && !String.IsNullOrWhiteSpace(docSelection.RefNbr))
            {
                ...
                else
                {
                    ...
                    Document_Detail.Delete(docSelection);
                    cache.RaiseExceptionHandling<DocumentSelection.refNbr>(docSelection, docSelection.RefNbr, new PXSetPropertyException(AP.Messages.ReferenceNotValid));
                }
            }
        }
...
namespace PX.Objects.AR.Overrides.ScheduleMaint
{
	...
	public partial class DocumentSelection : ARRegister
	{
		...
		[PXStringList(new string[] { ARDocType.Invoice }, new string[] { Messages.Invoice })]
		...
		[ARInvoiceType.RefNbr(typeof(Search<ARRegister.refNbr, Where<ARRegister.docType, Equal<Optional<ARRegister.docType>>, And<ARRegister.released, Equal<boolFalse>, And<ARRegister.hold, Equal<boolFalse>, And<ARRegister.voided,Equal<boolFalse>>>>>>))]
