	...
	public class AllocationMaint : PXGraph<AllocationMaint, PMAllocation>
	{
		...
		protected virtual void PMAllocationStep_AccountGroupOrigin_FieldSelecting(PXCache sender, PXFieldSelectingEventArgs e)
		{
			...
			if (row != null)
			{
				...
				allowedLabels.Add(Messages.Origin_Source);
				allowedLabels.Add(Messages.Origin_Change);

				if (row.UpdateGL == true)
				{
					allowedValues.Add(PMOrigin.FromAccount);
					allowedLabels.Add(Messages.Origin_FromAccount);
				}
				else if (row.Method == PMMethod.Budget)
				{
					allowedValues.Add(PMOrigin.None);
					allowedLabels.Add(Messages.Origin_None);
				}
		...
		protected virtual void PMAllocationStep_Reverse_FieldVerifying(PXCache sender, PXFieldVerifyingEventArgs e)
		{
			PMAllocationStep row = e.Row as PMAllocationStep;
			if (row != null)
			{
				string value = (string)e.NewValue;
				if (row.UpdateGL != true && value == PMReverse.OnInvoice)
				{
					e.NewValue = PMReverse.OnBilling;
				}
			}
		}
