...
{
	[Serializable]
	public class BusinessAccountGraphBase<Base, Primary, Where> : PXGraph<BusinessAccountGraphBase<Base, Primary, Where>>
		...
		where Where : class, IBqlWhere, new()
	{
		...
		public PXSelect<Base, Where2<Match<Current<AccessInfo.userName>>, And<Where>>> BAccount;
		public virtual PXSelectBase<Base> BAccountAccessor
		{
			get
			{
		...
		//public PXSelect<LocationExtAddress, Where<LocationExtAddress.locationBAccountID, Equal<Current<BAccount.bAccountID>>>> OtherLocations;
		...
		public PXSelect<BAccountItself, Where<BAccountItself.bAccountID, Equal<Optional<BAccount.bAccountID>>>> CurrentBAccountItself;
		...
		public virtual IEnumerable ViewContact(PXAdapter adapter)
		{
			if (this.ExtContacts.Current != null && this.BAccountAccessor.Cache.GetStatus(this.BAccountAccessor.Current) != PXEntryStatus.Inserted)
			{
				...
				//throw new PXRedirectRequiredException(graph, Messages.ContactMaint);
				throw new PXPopupRedirectException(graph, Messages.ContactMaint, true);
			}
			return adapter.Get();
		}
		...
		[PXUIField(DisplayName = Messages.ViewLocation,
			MapEnableRights = PXCacheRights.Select,
			MapViewRights = PXCacheRights.Select,
			Visible = false)]
		[PXButton(ImageKey = PX.Web.UI.Sprite.Main.DataEntry)]
		public virtual IEnumerable ViewLocation(PXAdapter adapter)
		{
			if (this.Locations.Current != null && this.BAccountAccessor.Cache.GetStatus(this.BAccountAccessor.Current) != PXEntryStatus.Inserted)
			{
				...
				LocationMaint graph = PXGraph.CreateInstance<LocationMaint>();
				graph.Location.Current = graph.Location.Search<Location.locationID>(current.LocationID, this.BAccountAccessor.Current.AcctCD);
				//throw new PXRedirectRequiredException(graph, Messages.LocationMaint);
				throw new PXPopupRedirectException(graph, Messages.LocationMaint, true);
			}
			return adapter.Get();
		}
		...
		{
			Base acct = this.BAccountAccessor.Current;
			LocationExtAddress loc = Locations.Current;
			if (loc != null && acct != null)
			{
				if (loc.IsActive != true)
				{
					Locations.Cache.RaiseExceptionHandling<LocationExtAddress.isActive>(loc, null, new PXSetPropertyException(CR.Messages.DefaultLocationCanNotBeNotActive, typeof(LocationExtAddress.isActive).Name));
				}
				else
				{
					SetAsDefault(loc);
		...
		protected virtual void Address_RowInserted(PXCache cache, PXRowInsertedEventArgs e)
		{
			if (e.ExternalCall && BAccountAccessor.Cache.GetStatus(BAccountAccessor.Current) == PXEntryStatus.Inserted)
			{
				BAccountAccessor.Current.DefAddressID = DetailInserted<Address>(cache, (Address)e.Row, BAccountAccessor.Current.DefAddressID);
			}
		}
	...
	[System.SerializableAttribute()]
	[PXPrimaryGraph(typeof(CR.BusinessAccountMaint))]
