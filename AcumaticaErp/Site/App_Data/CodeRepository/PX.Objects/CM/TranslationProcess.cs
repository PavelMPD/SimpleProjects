...
namespace PX.Objects.CM
{
    ...
    public class TranslationProcess : PXGraph<TranslationProcess>
    {
        ...
        public partial class TranslationParams : PX.Data.IBqlTable
        {
			...
			[PXUIField(DisplayName = "Branch", Visibility = PXUIVisibility.SelectorVisible, Enabled = false, Required = false)]
        ...
        public virtual IEnumerable translate(PXAdapter adapter)
        {
            ...
            if (existHistory != null)
                cache.RaiseExceptionHandling<TranslationParams.finPeriodID>(parameters, parameters.FinPeriodID, new PXSetPropertyException(Messages.ReleasedTranslationExistsInGreaterPeriod, existHistory.ReferenceNbr, PXErrorLevel.Warning));
        ...
        protected virtual void TranslationParams_LastFinPeriodID_FieldDefaulting(PXCache cache, PXFieldDefaultingEventArgs e)
        {
            ...
            {
                FinPeriod period = (FinPeriod)PXSelectJoin<FinPeriod, InnerJoin<TranslationHistory, On<TranslationHistory.finPeriodID, Equal<FinPeriod.finPeriodID>>>, Where<TranslationHistory.translDefId, Equal<Required<TranslationParams.translDefId>>>>.Select(cache.Graph, row.TranslDefId);
                if (period != null)
                {
    ...
    public class TranslationHistoryMaint : PXGraph<TranslationHistoryMaint>
    {
        ...
        //public PXAction<TranslationHistory> Reverse;
        ...
        /*
			#region ViewReversedBatch
			public PXAction<TranslationHistory> viewReversedBatch;
			[PXUIField(DisplayName = Messages.ViewTranslationBatch, MapEnableRights = PXCacheRights.Select, MapViewRights = PXCacheRights.Select)]
			[PXLookupButton]
			public virtual IEnumerable ViewReversedBatch(PXAdapter adapter)
			{
				if (TranslHistRecords.Current != null && TranslHistRecords.Current.ReversedBatchNbr != null)
				{
					JournalEntry graph = PXGraph.CreateInstance<JournalEntry>();
					graph.BatchModule.Current = graph.BatchModule.Search<Batch.batchNbr>(TranslHistRecords.Current.ReversedBatchNbr, GL.BatchModule.CM);
					throw new PXRedirectRequiredException(graph, Messages.ViewTranslationBatch);
				}
				return adapter.Get();
			}
        ...
        public TranslationHistoryMaint()
        {
            ...
            //TranslHistDetRecords.Cache.AllowDelete = false;
