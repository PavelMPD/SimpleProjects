...
public partial class Page_ShowWiki : ShowPage
{
	...
	private const string _SLOT_KEY_PREFIX = "WikiScript";
	private static readonly Type _templateWikiArticleType;
	private static readonly PropertyInfo _rootTemplate;

	public sealed class WikiScript : IPrefetchable
	{
		private Dictionary<Guid, List<Guid>> pagesAnlytics;
		private Dictionary<Guid, string> pagesAnlyticstext;
		
		public List<string> GetAllScript(Guid id)
		{
			List<string> ret = new List<string>();
			if (pagesAnlytics != null && pagesAnlytics.ContainsKey(id))
			{
				foreach (Guid scriptid in pagesAnlytics[id])
				{
		...
		{
			if (pagesAnlyticstext != null && pagesAnlyticstext.ContainsKey(id))
				return pagesAnlyticstext[id];
			return "";
		}
	...
	{
		_templateWikiArticleType = System.Web.Compilation.BuildManager.GetType(_TEMPLATE_WIKI_ARTICLE, false);
